...
Cada lote deve declarar grupo, ambiente, espécie, IDs únicos, nomes popular/científico, descrição, características, habitat, comportamento, alimentação, curiosidades, distribuição, importância ecológica e fontes estruturadas. Conservação só quando confirmada por ICMBio/MMA. Cada imagem precisa ser CC0/CC BY/CC BY-SA confirmada no arquivo específico, com autor, licença, crédito e URL. Proibidos NC/ND, IUCN e URLs inventadas; marque pendências. No Ciclo 1 entregue lote piloto pequeno, informe a vazão validada e recalibre o tamanho a partir do Ciclo 3. Use arquivos modulares por lote e mantenha um índice. Rode `pnpm check`, `pnpm lint`, `pnpm test`, `git diff --check`, atualize `todo.md`, informe quantidade e cobertura do lote e abra um PR pequeno. Não faça merge nem altere `main`.
...
Agente 1 integra. Agente 2 adiciona dados. Agente 3 melhora produto e qualidade. Dependências: contratos antes dos lotes; lotes antes das telas dependentes; componentes nativos separados antes do mapa; testes antes do merge. Cada PR informa arquivos, quantidade de espécies, testes, riscos, próxima dependência e auditoria de licenças. O PR é bloqueado se faltar licença comercial confirmada, fonte aprovada, crédito, ID único ou status oficial de conservação. Nenhum agente sobrescreve trabalho alheio, usa reset destrutivo ou mistura dados temporários de colaboração com dados do usuário.
