...
export function RemoteImage({ label, style, accessibilityLabel, source }: RemoteImageProps) {
  ...
  return (
    <View style={[style, { backgroundColor: colors.surface, overflow: "hidden" }]}>
      {status !== "error" && <ExpoImage source={source} accessibilityLabel={accessibilityLabel ?? label} cachePolicy="disk" contentFit="cover" transition={200} onLoad={() => setStatus("ready")} onError={() => setStatus("error")} style={[StyleSheet.absoluteFillObject, { opacity: status === "ready" ? 1 : 0 }]} />}
