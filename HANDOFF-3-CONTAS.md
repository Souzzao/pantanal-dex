...
## Retomada — seleção prioritária do catálogo

A retomada solicitada para o item 1/50 foi reconciliada com o estado real da branch: os passos 1–13/50 já estavam registrados como concluídos e não foram repetidos. Como melhoria verificável da seleção, `shared/catalog/priorities.ts` agora exige uma consulta taxonômica GBIF por prioridade, rejeita IDs prioritários duplicados ou fora de kebab-case, detecta táxons científicos repetidos e, quando recebe o catálogo combinado, confere nome científico, grupo e ambientes do registro correspondente.

A matriz foi corrigida para eliminar o duplicado sintético `Dourado-fish06`/`Salminus brasiliensis`, substituindo-o por `Traíra`/`Hoplias malabaricus`, e para alinhar os ambientes e nomes científicos divergentes encontrados em Anhuma, Anta, Tamanduá-bandeira, Sapo-cururu, Dourado, Martim-pescador-grande, Cobra-cipó e Perereca. O artefato `CATALOG-PRIORITY-SELECTION.md`, gerado por `pnpm catalog:priority-audit`, registra 41 entradas prioritárias, 36 selecionadas com ID no catálogo combinado, 5 pendentes sem registro e `status=PASS`. Nenhuma prioridade foi promovida a `verified`; ocorrência regional, imagens/licenças e conservação continuam pendentes de revisão editorial.

Arquivos desta retomada: `shared/catalog/priorities.ts`, `shared/catalog/index.ts`, `scripts/catalog-priority-audit.ts`, `tests/pantanal.catalog.test.ts`, `package.json` e `CATALOG-PRIORITY-SELECTION.md`. A matriz mantém fonte GBIF estruturada para nomenclatura e não usa essa fonte como prova de ocorrência pantaneira.

Próximo bloco desbloqueado da fila da Conta 2 permanece `catalog-birds-06` ou `catalog-fish-07`; o Passo 1/50 histórico continua concluído e não deve ser reaberto sem evidência nova.

Validação final desta retomada: `pnpm mvp:report` registrou 20 espécies públicas, 55 modulares, 75 totais, 21 lotes `pending-review`, 0 `verified`, 0 `review-ready`, 20 fontes públicas, 55 fontes por espécie modular, 21 fontes por lote, 75 IDs únicos e nenhum duplicado. `pnpm catalog:priority-audit`, `pnpm catalog:image-audit` (165/165 licenças aprovadas), `pnpm catalog:source-audit` (55 espécies, 110 URLs GBIF, 0 erros), `pnpm check`, `pnpm lint`, `pnpm test` (18 aprovados e 1 legado ignorado), `pnpm watchdog` e `git diff --check` passaram. O lint emite apenas o aviso existente sobre `type: module` ausente no `package.json`; não há erro de lint.

## Retomada adicional — Passo 2/50 com validação nativa

O Passo 2 foi executado novamente de forma incremental e verificável. Foi criado `shared/catalog/metrics.ts`, com cálculo compartilhado de espécies públicas/modulares, total, IDs únicos e duplicados, lotes por status, imagens, grupos, ambientes e erros de validação. A tela `app/(tabs)/settings.tsx` agora exibe um cartão nativo “Inventário do catálogo”, identificado como Passo 2/50, com os números reais do acervo e distribuição por grupo. A validação no My Browser confirmou a renderização de 75 espécies totais, 75 IDs únicos, 20 públicas, 55 modulares, 21 lotes, catálogo estruturalmente válido e 165 imagens. O relatório textual também foi corrigido para não contar o `group` do cabeçalho de cada lote como uma espécie.

A rodada de validação passou `pnpm check`, `pnpm test -- tests/pantanal.catalog.test.ts` (19/19), `pnpm mvp:report`, `pnpm catalog:baseline`, `pnpm catalog:priority-audit`, `pnpm catalog:image-audit`, `pnpm catalog:source-audit`, `pnpm lint`, `pnpm test`, `pnpm watchdog` e `git diff --check`. As alterações deste ciclo ainda precisam ser commitadas e publicadas no PR #11.

## Retomada adicional — Passo 3/50: baseline canônico
