...
describe("PantanalDex catalog", () => {
  ...
  it("keeps documented synonyms traceable and searchable", () => {
    expect(documentedSynonymValidationErrors).toEqual([]);
    expect(Object.keys(documentedSynonyms)).toHaveLength(5);
    const combinedWithSynonyms = applyDocumentedSynonyms([...species, ...catalogSpecies]);
    expect(combinedWithSynonyms.find((item) => item.id === "pintado")?.searchNames).toContain("Platystoma corruscans");
    expect(combinedWithSynonyms.find((item) => item.id === "tuiuiu")?.searchNames).toContain("Ciconia mycteria");
    expect(catalogSpeciesWithSynonyms.every((item) => new Set(item.searchNames ?? []).size === (item.searchNames ?? []).length)).toBe(true);
  });
  ...
  it("keeps regional occurrence evidence conservative and traceable", () => {
    ...
    expect(regionalOccurrenceRecords.filter((record) => record.speciesId !== "pintado" && record.speciesId !== "pacu" && record.speciesId !== "piraputanga").every((record) => record.status === "pending-review")).toBe(true);
    ...
    expect(regionalOccurrenceRecords.filter((record) => record.speciesId === "pintado" || record.speciesId === "pacu" || record.speciesId === "piraputanga").every((record) => record.evidence.includes("GBIF/Catalogue of Life"))).toBe(true);
    expect(regionalOccurrenceRecords.filter((record) => record.speciesId !== "pintado" && record.speciesId !== "pacu" && record.speciesId !== "piraputanga").every((record) => record.sourceTitle.includes("ICMBio/SISBio") && record.evidence.includes("não confirma ocorrência"))).toBe(true);
  });
