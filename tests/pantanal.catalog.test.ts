...
describe("PantanalDex catalog", () => {
  ...
  it("keeps conservation review official and conservative", () => {
    expect(conservationReviewRecords).toHaveLength(21);
    expect(conservationReviewValidationErrors).toEqual([]);
    expect(conservationReviewRecords.find((record) => record.speciesId === "boana-albopunctata")).toMatchObject({ status: "confirmed", finding: "not-listed", sourceKind: "Portaria MMA/ICMBio" });
    ...
    expect(conservationReviewRecords.find((record) => record.speciesId === "hoplias-malabaricus")).toMatchObject({ status: "confirmed", finding: "not-listed", sourceKind: "Portaria MMA/ICMBio" });
    ...
    expect(conservationReviewRecords.find((record) => record.speciesId === "zungaro-jahu")).toMatchObject({ status: "confirmed", finding: "not-listed", sourceKind: "Portaria MMA/ICMBio" });
  ...
  it("integrates the modular catalog batches without validation errors", () => {
    ...
    expect(catalogSpecies.some((item) => item.id === "phoneutria-nigriventer")).toBe(true);
