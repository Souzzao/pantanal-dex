...
export default function HomeScreen() {
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background">
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
      ...
      <View style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: 12, marginBottom: 12 }} accessibilityRole="summary"><Text style={{ color: colors.foreground, fontWeight: "800" }}>Catálogo disponível offline</Text><Text style={{ color: colors.muted, marginTop: 3 }}>Pesquise espécies e consulte as fichas mesmo sem conexão.</Text></View>
      <TextInput accessibilityLabel="Buscar animal pelo nome popular ou científico" value={query} onChangeText={setQuery} placeholder="Buscar animal pelo nome" placeholderTextColor={colors.muted} style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 16, paddingHorizontal: 16, paddingVertical: 14, color: colors.foreground, fontSize: 15 }} />
      <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}><Pressable onPress={() => router.push("/(tabs)/animals" as any)} accessibilityRole="button" accessibilityLabel="Explorar animais" style={({ pressed }) => [{ flex: 1, backgroundColor: colors.primary, padding: 16, borderRadius: 16 }, pressed && { opacity: .8 }]}><Text style={{ color: "#fff", fontWeight: "700", textAlign: "center" }}>Explorar animais</Text></Pressable><Pressable onPress={() => router.push("/(tabs)/sightings" as any)} accessibilityRole="button" accessibilityLabel="Meus avistamentos" style={({ pressed }) => [{ flex: 1, backgroundColor: colors.primary, padding: 16, borderRadius: 16 }, pressed && { opacity: .8 }]}><Text style={{ color: "#fff", fontWeight: "700", textAlign: "center" }}>Meus avistamentos</Text></Pressable></View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 28, marginBottom: 12 }}><Text style={{ color: colors.foreground, fontSize: 22, fontWeight: "800" }}>Em destaque</Text><Pressable onPress={() => router.push("/(tabs)/animals" as any)} accessibilityRole="button" accessibilityLabel="Ver todas as espécies"><Text style={{ color: colors.primary, fontWeight: "700" }}>Ver todos</Text></Pressable></View>
      {highlights.map((item) => <Pressable key={item.id} onPress={() => router.push({ pathname: "/species/[id]", params: { id: item.id } } as any)} style={({ pressed }) => [{ backgroundColor: colors.surface, borderRadius: 18, marginBottom: 12, overflow: "hidden", borderWidth: 1, borderColor: colors.border }, pressed && { opacity: .8 }]}><RemoteImage source={{ uri: item.images[0].uri }} label={item.commonName} style={{ width: "100%", height: 150, backgroundColor: colors.border }} /><View style={{ padding: 14 }}><Text style={{ color: colors.foreground, fontSize: 18, fontWeight: "800" }}>{item.commonName}</Text><Text style={{ color: colors.muted, fontStyle: "italic", marginTop: 3 }}>{item.scientificName}</Text><Text style={{ color: colors.primary, marginTop: 8, fontWeight: "700" }}>{item.group} · {item.environments[0]}</Text></View></Pressable>)}
    </ScrollView>
  </ScreenContainer>;
}
