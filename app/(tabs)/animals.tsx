...
export default function AnimalsScreen() {
  ...
  return <ScreenContainer className="px-5">
    ...
    <FlatList data={filtered} keyExtractor={(item) => item.id} contentContainerStyle={{ paddingBottom: 30 }} ListEmptyComponent={<Text style={{ color: colors.muted, textAlign: "center", paddingTop: 40 }}>Nenhum animal encontrado com esses filtros.</Text>} renderItem={({ item }) => <Pressable onPress={() => router.push({ pathname: "/species/[id]", params: { id: item.id } } as any)} style={({ pressed }) => [{ flexDirection: "row", backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1, borderColor: colors.border, marginBottom: 12, overflow: "hidden" }, pressed && { opacity: 0.8 }]}><Image source={{ uri: item.images[0].uri }} style={{ width: 112, height: 112, backgroundColor: colors.border }} /><View style={{ flex: 1, padding: 14 }}><Text style={{ color: colors.foreground, fontSize: 17, fontWeight: "800" }}>{item.commonName}</Text><Text style={{ color: colors.muted, fontStyle: "italic", marginTop: 4 }}>{item.scientificName}</Text><Text style={{ color: colors.primary, fontSize: 12, fontWeight: "700", marginTop: 12 }}>{item.group} · {item.environments[0]}</Text></View></Pressable>} />
  </ScreenContainer>;
}
