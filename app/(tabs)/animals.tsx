...
export default function AnimalsScreen() {
  ...
  return <ScreenContainer className="px-5">
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>{["", ...environments].map((item) => <View key={`e-${item}`}>{chip(item || "Todos os ambientes", environment === item, () => setEnvironment(item))}</View>)}</View>
