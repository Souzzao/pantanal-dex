...
export default function SettingsScreen() {
  ...
  const labels = english ? { title: "Settings", subtitle: "Adapt PantanalDex to your field work", ready: "Notebook available offline", restoring: "Restoring local notebook…", loaded: "records and preferences loaded on this device", wait: "Wait a moment before editing or exporting records.", storageIssue: "Some local data needs attention. The recovered records remain available; export a backup before clearing storage.", reviewTitle: "Catalog under editorial review", reviewDetail: (species: number, batches: number) => `${species} species in ${batches} batch(es) await license and source audit.` } : spanish ? { title: "Configuración", subtitle: "Adapta PantanalDex a tu trabajo de campo", ready: "Cuaderno disponible sin conexión", restoring: "Restaurando cuaderno local…", loaded: "registro(s) y preferencias cargados en este dispositivo", wait: "Espera antes de editar o exportar registros.", storageIssue: "Algunos datos locales requieren atención. Los registros recuperados siguen disponibles; exporta una copia antes de borrar el almacenamiento.", reviewTitle: "Catálogo en revisión editorial", reviewDetail: (species: number, batches: number) => `${species} especie(s) en ${batches} lote(s) esperan auditoría de licencia y fuente.` } : { title: "Configurações", subtitle: "Ajuste o PantanalDex ao seu campo", ready: "Caderno disponível offline", restoring: "Restaurando caderno local…", loaded: "registro(s) e preferências carregados neste aparelho.", wait: "Aguarde alguns instantes antes de editar ou exportar registros.", storageIssue: "Alguns dados locais precisam de atenção. Os registros recuperados continuam disponíveis; exporte um backup antes de limpar o armazenamento.", reviewTitle: "Catálogo em revisão editorial", reviewDetail: (species: number, batches: number) => `${species} espécie(s) em ${batches} lote(s) aguardam auditoria de licença e fonte.` };
