...
export default function NewSightingScreen() {
  ...
  const cameraDenied = Platform.OS !== "web" && cameraPermission?.status === "denied";
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5">
    <ScrollView contentContainerStyle={{ paddingBottom: 35 }}>
      ...
      {Platform.OS === "web" ? <View accessibilityRole="summary" style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: 14, marginBottom: 12 }}><Text style={{ color: colors.foreground, fontWeight: "800" }}>Captura no navegador</Text><Text style={{ color: colors.muted, marginTop: 5, lineHeight: 19 }}>A câmera nativa está disponível no aplicativo para celular. No navegador, use a galeria para escolher uma fotografia.</Text><Pressable onPress={pickPhoto} accessibilityRole="button" accessibilityLabel="Escolher fotografia no navegador" style={{ alignSelf: "flex-start", marginTop: 10 }}><Text style={{ color: colors.primary, fontWeight: "800" }}>Escolher na galeria</Text></Pressable></View> : null}
      {cameraDenied ? <View accessibilityRole="alert" style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: 14, marginBottom: 12 }}><Text style={{ color: colors.foreground, fontWeight: "800" }}>Permissão da câmera negada</Text><Text style={{ color: colors.muted, marginTop: 5, lineHeight: 19 }}>{cameraPermission?.canAskAgain ? "Você pode tentar autorizar novamente ou usar a galeria." : "A permissão foi bloqueada. Abra as configurações do aparelho para autorizá-la ou use a galeria."}</Text><View style={{ flexDirection: "row", gap: 14, marginTop: 10 }}>{cameraPermission?.canAskAgain ? <Pressable onPress={openCamera} accessibilityRole="button" accessibilityLabel="Tentar autorizar câmera"><Text style={{ color: colors.primary, fontWeight: "800" }}>Tentar novamente</Text></Pressable> : <Pressable onPress={() => Linking.openSettings()} accessibilityRole="button" accessibilityLabel="Abrir configurações da câmera"><Text style={{ color: colors.primary, fontWeight: "800" }}>Abrir configurações</Text></Pressable>}<Pressable onPress={pickPhoto} accessibilityRole="button" accessibilityLabel="Usar galeria após negar câmera"><Text style={{ color: colors.primary, fontWeight: "800" }}>Usar galeria</Text></Pressable></View></View> : null}
      {cameraOpen ? <View style={{ height: 300, borderRadius: 16, overflow: "hidden", marginBottom: 12, backgroundColor: "#111" }}><CameraView ref={cameraRef} facing="back" onCameraReady={() => setCameraReady(true)} onMountError={(event) => { setCameraOpen(false); setCameraReady(false); setMediaError(`A câmera não pôde ser iniciada: ${event.message}`); }} style={{ flex: 1 }} /><View style={{ position: "absolute", left: 0, right: 0, bottom: 14, flexDirection: "row", justifyContent: "center", gap: 10 }}><Pressable onPress={() => { setCameraOpen(false); setCameraReady(false); }} accessibilityRole="button" accessibilityLabel="Cancelar câmera" style={{ backgroundColor: "rgba(0,0,0,.65)", borderRadius: 18, paddingHorizontal: 16, paddingVertical: 11 }}><Text style={{ color: "#fff", fontWeight: "800" }}>Cancelar</Text></Pressable><Pressable onPress={takePhoto} disabled={!cameraReady || isBusy} accessibilityRole="button" accessibilityLabel="Capturar fotografia" style={{ backgroundColor: cameraReady ? colors.primary : colors.muted, borderRadius: 18, paddingHorizontal: 18, paddingVertical: 11 }}><Text style={{ color: "#fff", fontWeight: "800" }}>{cameraReady ? "Capturar" : "Preparando…"}</Text></Pressable></View></View> : null}
