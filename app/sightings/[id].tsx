...
export default function SightingDetailScreen() {
  ...
  if (!sighting || !animal) {
    return <ScreenContainer className="px-5"><Text style={{ color: colors.foreground, paddingTop: 24 }}>Registro não encontrado.</Text></ScreenContainer>;
  }
  ...
  return (
    <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5">
      <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
        ...
        {sighting.photoUri ? <Image source={{ uri: sighting.photoUri }} style={{ width: "100%", height: 230, borderRadius: 18 }} /> : <Image source={{ uri: animal.images[0].uri }} style={{ width: "100%", height: 230, borderRadius: 18, backgroundColor: colors.border }} />}
        ...
        <View style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 16, padding: 16, marginTop: 20 }}>
          ...
          {sighting.latitude !== undefined && <Text style={{ color: colors.muted, marginTop: 5 }}>Coordenadas: {sighting.latitude.toFixed(4)}, {sighting.longitude?.toFixed(4)}</Text>}
          ...
          <Text style={{ color: colors.muted, marginTop: 6 }}>{sighting.visibility === "shareable" ? "Compartilhável" : "Pessoal"}</Text>
        ...
        <Pressable onPress={() => router.push({ pathname: "/sightings/new", params: { id: sighting.id } } as any)} style={({ pressed }) => [{ backgroundColor: colors.primary, borderRadius: 15, padding: 15, marginTop: 20 }, pressed && { opacity: 0.82, transform: [{ scale: 0.98 }] }]}>
          <Text style={{ color: "#fff", textAlign: "center", fontWeight: "800" }}>Editar registro</Text>
        </Pressable>
        <Pressable onPress={confirmDelete} style={{ borderColor: colors.error, borderWidth: 1, borderRadius: 15, padding: 15, marginTop: 12 }}>
