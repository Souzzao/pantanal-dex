...
export default function SpeciesDetailScreen() {
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
    ...
    <Text accessibilityRole="header" style={{ color: colors.muted, fontWeight: "700", marginTop: 24 }}>Imagens e fontes</Text>{animal.images.map((image) => <Pressable key={`${image.uri}-credit`} onPress={() => Linking.openURL(image.sourceUrl)} accessibilityRole="link"><Text style={{ color: colors.muted, fontSize: 12, marginTop: 6 }}>{image.credit} · {image.license}</Text></Pressable>)}{animal.sources.map((source) => <Pressable key={source.url} onPress={() => Linking.openURL(source.url)} accessibilityRole="link"><Text style={{ color: colors.primary, marginTop: 8 }}>{source.title}</Text></Pressable>)}
