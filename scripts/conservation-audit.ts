...
const expectedIds = ["tucano-toco", "arara-caninde", "veado-campeiro", "pintado", "pacu", "piraputanga", "caranguejo-agua-doce", "camarao-agua-doce"];
...
const markdown = [
  ...
  "",
  "> Nenhuma categoria de ameaça foi inventada. `tucano-toco` tem LC em ficha específica do SALVE/ICMBio; `arara-canindé` tem finding `not-listed` na Portaria MMA nº 148/2022; `pintado` tem VU na mesma Portaria; `pacu` e `piraputanga` têm finding `not-listed`; `caranguejo-agua-doce` e `camarao-agua-doce` têm LC nas avaliações técnicas oficiais do ICMBio. O veado-campeiro tem VU na ficha oficial do SALVE, cuja unidade avaliada é a subespécie O. b. bezoarticus. As avaliações LC são datadas e não devem ser confundidas automaticamente com uma lista legal posterior.",
  "",
].join("\n");
