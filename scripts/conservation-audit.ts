...
const expectedIds = ["bombus-pauloensis", "leporinus-obtusidens", "boana-albopunctata", "oxybelis-aeneus", "pitangus-sulphuratus", "pimelodus-maculatus", "vanessa-myrinna", "hoplias-malabaricus", "prochilodus-lineatus", "megaceryle-torquata", "egretta-thula", "heliconius-erato", "cateto", "phoneutria-nigriventer", "zungaro-jahu", "tucano-toco", "arara-caninde", "veado-campeiro", "pintado", "pacu", "piraputanga", "caranguejo-agua-doce", "camarao-agua-doce", "brycon-orbignyanus", "serrasalmus-maculatus", "serrasalmus-marginatus", "gymnotus-inaequilabiatus", "eigenmannia-virescens", "rhamdia-quelen", "synbranchus-marmoratus", "crenicichla-britskii", "hemisorubim-platyrhynchos", "loricariichthys-platymetopon", "paleosuchus-palpebrosus", "micrablepharus-maximiliani", "phrynops-geoffroanus", "podocnemis-unifilis", "hydrodynastes-gigas"];
...
const mamangava = conservationReviewRecords.find((record) => record.speciesId === "bombus-pauloensis");
const piava = conservationReviewRecords.find((record) => record.speciesId === "leporinus-obtusidens");
const pererecaBanheiro = conservationReviewRecords.find((record) => record.speciesId === "boana-albopunctata");
const cobraCipo = conservationReviewRecords.find((record) => record.speciesId === "oxybelis-aeneus");
const bemTeVi = conservationReviewRecords.find((record) => record.speciesId === "pitangus-sulphuratus");
const mandi = conservationReviewRecords.find((record) => record.speciesId === "pimelodus-maculatus");
const pintadaSul = conservationReviewRecords.find((record) => record.speciesId === "vanessa-myrinna");
const traira = conservationReviewRecords.find((record) => record.speciesId === "hoplias-malabaricus");
...
if (!mamangava || mamangava.status !== "confirmed" || mamangava.finding !== "not-listed" || mamangava.sourceKind !== "Portaria MMA/ICMBio" || !mamangava.evidence.includes("Bombus pauloensis")) errors.push("mamangava-de-chão não possui finding oficial conservador de não listagem");
if (!piava || piava.status !== "confirmed" || piava.finding !== "not-listed" || piava.sourceKind !== "Portaria MMA/ICMBio" || !piava.evidence.includes("Leporinus obtusidens")) errors.push("piava não possui finding oficial conservador de não listagem");
if (!pererecaBanheiro || pererecaBanheiro.status !== "confirmed" || pererecaBanheiro.finding !== "not-listed" || pererecaBanheiro.sourceKind !== "Portaria MMA/ICMBio" || !pererecaBanheiro.evidence.includes("Boana albopunctata")) errors.push("perereca-do-banheiro não possui finding oficial conservador de não listagem");
if (!cobraCipo || cobraCipo.status !== "confirmed" || cobraCipo.finding !== "not-listed" || cobraCipo.sourceKind !== "Portaria MMA/ICMBio" || !cobraCipo.evidence.includes("Oxybelis aeneus")) errors.push("cobra-cipó não possui finding oficial conservador de não listagem");
if (!bemTeVi || bemTeVi.status !== "confirmed" || bemTeVi.finding !== "not-listed" || bemTeVi.sourceKind !== "Portaria MMA/ICMBio" || !bemTeVi.evidence.includes("Pitangus sulphuratus")) errors.push("bem-te-vi não possui finding oficial conservador de não listagem");
if (!mandi || mandi.status !== "confirmed" || mandi.finding !== "not-listed" || mandi.sourceKind !== "Portaria MMA/ICMBio" || !mandi.evidence.includes("Pimelodus maculatus")) errors.push("mandi não possui finding oficial conservador de não listagem");
if (!pintadaSul || pintadaSul.status !== "confirmed" || pintadaSul.finding !== "not-listed" || pintadaSul.sourceKind !== "Portaria MMA/ICMBio" || !pintadaSul.evidence.includes("Vanessa myrinna")) errors.push("pintada-do-sul não possui finding oficial conservador de não listagem");
if (!traira || traira.status !== "confirmed" || traira.finding !== "not-listed" || traira.sourceKind !== "Portaria MMA/ICMBio" || !traira.evidence.includes("Hoplias malabaricus")) errors.push("traíra não possui finding oficial conservador de não listagem");
