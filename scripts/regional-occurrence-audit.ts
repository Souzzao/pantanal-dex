...
if (!ar || ar.status !== "confirmed" || !ar.sourceTitle.includes("SciELO") || !ar.evidence.includes("GBIF/Catalogue of Life")) errors.push("aranha-armadeira não possui promoção sustentada por evidência regional e taxonômica");
  if (regionalOccurrenceRecords.some((record) => record.status !== "confirmed" && record.status !== "pending-review" && record.status !== "not-confirmed")) errors.push("registro regional possui estado inválido");
...
const markdown = [
  ...
  "",
  `**Resultado:** ${errors.length ? "FAIL" : "PASS"}. ${regionalOccurrenceRecords.length}/${expectedIds.length} registros cobertos; ${regionalOccurrenceRecords.filter((record) => record.status === "confirmed").length} confirmado(s); ${regionalOccurrenceRecords.filter((record) => record.status === "pending-review").length} pendência(s) aberta(s); ${regionalOccurrenceRecords.filter((record) => record.status === "not-confirmed").length} não confirmado(s); ${errors.length} erro(s).`,
  "",
